...
const PrincipalCard = ({ name, prediction, image }) => {
  return (
    <Link className="w-full flex justify-center" to={`/sign/${name.toLowerCase()}`}>
      <div
        className={`container group p-4 flex cursor-pointer items-start overflow-hidden rounded-lg border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-xl h-40 gap-1 sm:h-44 md:h-48 w-full lg:w-[90%] xl:w-[80%] sm:gap-2 `}
      >
        <img
          src={`/${image}`}
